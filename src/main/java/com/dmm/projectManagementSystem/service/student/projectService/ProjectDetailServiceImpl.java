...
public class ProjectDetailServiceImpl implements ProjectDetailService {
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public ApiResponseStudent<List<com.dmm.projectManagementSystem.dto.meeting.MeetingResponse>> handleGetMeeting(
      Long topicId, int page, int size, String sort) {
    ...
    // Convert Meeting entity list to MeetingResponse DTO list
    List<com.dmm.projectManagementSystem.dto.meeting.MeetingResponse> meetingDTOs = pageList.getContent().stream()
        .map(com.dmm.projectManagementSystem.dto.meeting.MeetingResponse::fromMeeting)
        .toList();

    ApiResponseStudent<List<com.dmm.projectManagementSystem.dto.meeting.MeetingResponse>> apiResponseGetMeeting = new ApiResponseStudent<>();
    apiResponseGetMeeting.setMetadata(metadata);
    apiResponseGetMeeting.setMessage("Lấy lịch họp thành công");
    apiResponseGetMeeting.setData(meetingDTOs);
  ...
  @Override
  @Transactional(rollbackFor = Exception.class)
  public ApiResponseStudent<List<com.dmm.projectManagementSystem.dto.task.TaskResponse>> handleGetTask(Long topicId,
    ...
    // Convert Task entity list to TaskResponse DTO list
    List<com.dmm.projectManagementSystem.dto.task.TaskResponse> taskDTOs = pageList.getContent().stream()
        .map(com.dmm.projectManagementSystem.dto.task.TaskResponse::fromTask)
        .toList();

    ApiResponseStudent<List<com.dmm.projectManagementSystem.dto.task.TaskResponse>> apiResponseGetTask = new ApiResponseStudent<>();
    apiResponseGetTask.setMetadata(metadata);
    apiResponseGetTask.setMessage("Lấy nhiệm vụ được giao thành công");
    apiResponseGetTask.setData(taskDTOs);
